...
class InfoModalController {
    ...
    renderTextFields(modalDataTextFields) {
        const textFieldsArea = this.modalContainer.querySelector('.modal_texts_fields_area');
        if (modalDataTextFields.length === 0) {
            textFieldsArea.innerHTML = '';
            return false;
        }
        let heading = `<h5 class="font-weight-bold">Text fields:</h5><hr/>`;
        let html = modalDataTextFields.reduce((acc, item) => {
